...
public class EmailUtil {
	...
	private String mail_host = "smtp.126.com";//ex:smtp.126.com
	private int mail_port = 25;
	private int encryptionType = EncryptionTypes.Default.ordinal();
	private boolean auth = true;
	private String mail_host_account = "workemailgong@126.com";//ex:workemailgong@126.com
	private String mail_host_password = "workemail66";//ex:workemail66
	private boolean isHtml = true;
	private String mail_subject = "sub";//邮件主题
	...
	public void sendEmailWithPicture(List<String> imgList, String receiverAddress, String msg) throws Exception {
		String[] address = receiverAddress.split(";");
		List<String> recipients = new ArrayList<String>();
		for (int i = 0; i < address.length; i++) {
			if (address[i].trim().length() > 0) {
		...
		this.finalsendEmailWithPicture(imgList, recipients, msg, null);
...
//				transport.sendMessage(EmailUtil.createImageMail(session), EmailUtil.createImageMail(session).getAllRecipients());
	...
	public void finalsendEmailWithPicture(List<String> imgList, List<String> recipients, String msg, 
		...
		try {
			...
			message.setSubject(mail_subject, default_charset);
...
//				string
//				);
		email.sendEmailWithPicture(imgList, "378476772@qq.com;18768104912@163.com;", string);
